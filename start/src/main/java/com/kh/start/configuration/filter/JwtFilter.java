...
//			log.info(username);
			...
			UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(user,null, user.getAuthorities()); // authentication에 담음
			authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request)); // 세부 설정 (사용자의 IP 주소, MAC 주소, SessionId 등등이 포함될 수 있음)
			
			SecurityContextHolder.getContext().setAuthentication(authentication);
			// session.setAttribute("loginMember", 사용자 정보); 이런 코드임 위는
